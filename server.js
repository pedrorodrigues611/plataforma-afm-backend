...
app.post('/api/forgot-password', async (req, res) => {
  ...
  try {
    ...
    console.error('Erro em POST /api/forgot-password:', err);
...
// Handle preflight for profile update
...
app.put('/api/profile', upload.single('photo'), async (req, res) => {
  ...
  try {
    ...
    console.error('Erro em PUT /api/profile:', err);
...
app.put('/api/make-admin', async (req, res) => {
  try {
    ...
    console.error('Erro em PUT /api/make-admin:', err);
    res.status(500).json({ message: 'Erro interno do servidor' });
  }
});
...
// Mount welcome or health-check
app.get('/', (req, res) => res.send('Servidor AFM UP'));

// Mount routers for other models if any
app.use('/api/relatorios', relatoriosRoutes);
app.use('/api/perguntas', perguntasRoutes);
app.use('/api/suggestions', suggestionsRoutes);
app.use('/api/users', usersRoutes);
